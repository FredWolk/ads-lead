...
@section('content')
    ...
    <section class="videopage articlepage eventpage">
        <div class="container">
            <div style="margin-bottom: 0;" class="articlepage_top">
                <div style="margin-bottom: 0;" class="eventpage_top">
                    <h1 class="title">{{ $vacinci["name"] }}</h1>
                </div>
            </div>
            <div class="videopage_wrapper">
                <article class="videopage_main">
                    <div style="justify-content: space-around" class="eventpage_card">
                        <div class="eventpage_card--item">
                            <p class="eventpage_card--item-title">Date</p>
                            <p class="eventpage_card--item-text">{{ date('d/m/Y', strtotime($vacinci["created_at"])) }}</p>
                        </div>
                        <div class="eventpage_card-line"></div>
                        <div class="eventpage_card--item">
                            <p class="eventpage_card--item-title">Location</p>
                            <p class="eventpage_card--item-text">{{ $vacinci["location"] }}</p>
                        </div>
                        <div class="eventpage_card-line"></div>
                        <div class="eventpage_card--item">
                            @php
                                if (!empty($vacinci['form_social']))
                                    $arr = explode('||', $vacinci['form_social'])
                            @endphp
                            <p class="eventpage_card--item-title">Contact</p>
                            <div class="eventpage_card--item_links">
                                @if(!empty($vacinci['linkedin']))
                                    <a class="eventpage_card--item--link" target="_blank"
                                       href="{{ $vacinci['linkedin'] }}">
                                        Linkedin
                                    </a>
                                @endif
                                @if(!empty($vacinci['e_mail']))
                                    <a class="eventpage_card--item--link" target="_blank"
                                       href="mailto: {{ $vacinci['e_mail'] }}">
                                        E-mail
                                    </a>
                                @endif
                                @if(!empty($vacinci['telegram']))
                                    <a class="eventpage_card--item--link" target="_blank"
                                       href="{{ $vacinci['telegram'] }}">
                                        Telegram
                                    </a>
                                @endif
                            </div>
                    ...
                    <div class="videopage_main--text main__article"
                         style="margin-bottom: 20px">{!! $vacinci["text"] !!}</div>
                    ...
                    @if(!empty($banner))
                        <div class="filter-aside--sticky">
                            <div class="filter-aside-banner sticky">
                                <a data-type="aside_banner" class="banner_check" target="_blank"
                                   href="{{ $banner->link }}">
